...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	cell_shader_ = new Shader("data/shaders/cell.vert", "data/shaders/cell.frag");
	heat_shader_ = new Shader("data/shaders/heat.vert", "data/shaders/heat.frag");
	flip_shader_ = new Shader("data/shaders/flip.vert", "data/shaders/flip.frag");
	dist_shader_ = new Shader("data/shaders/dist.vert", "data/shaders/dist.frag");
...
void GraphicsSystem::update(float dt) {
	...
	glViewport(0, 0, frame_.width/2, frame_.height/2);

	useShader(screen_space_shader_);
	useShader(cell_shader_);
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, frame_.color_textures[0], 0);

	geometries_[screen_space_geom_].render();

	glViewport(0, frame_.height/2, frame_.width / 2, frame_.height / 2);

	useShader(screen_space_shader_);
	useShader(flip_shader_);
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, frame_.color_textures[0], 0);

	geometries_[screen_space_geom_].render();

	glViewport(frame_.width / 2, 0, frame_.width / 2, frame_.height / 2);

	useShader(screen_space_shader_);
	useShader(dist_shader_);
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, frame_.color_textures[0], 0);

	geometries_[screen_space_geom_].render();

	glViewport(frame_.width / 2, frame_.height / 2, frame_.width/2, frame_.height/2);

	useShader(screen_space_shader_);
	useShader(heat_shader_);
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, frame_.color_textures[0], 0);
